...
async def make_changes_command(message: types.Message):
    ...
    await bot.send_message(message.from_user.id, "Начнем работу", reply_markup=admin_kb.button_case_admin)
...
# Получаем ответ картинки и пишем в словарь
...
async def del_callback_run(callback_queru: types.CallbackQuery):
    ...
    await callback_queru.answer(text=f'{callback_queru.data.replace("del ", "")} Запись удалена', show_alert=True)
...
async def delete_item(message: types.Message):
    if message.from_user.id == ID:
        ...
        for ret in read:
            await bot.send_photo(message.from_user.id, ret[0], f'{ret[1]}\Описание: {ret[2]}\nЦена {ret[-1]}')
